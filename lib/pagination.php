...
class Pagination
{
	...
	private function buildHTML($currentPage, $totalPages) {
		...
		if ($currentPage > 1) {
			$html .= '<li class="prevpage" data-page="'.($currentPage-1).'"><a href="#"> << </a></li>';
		}


		for ($i = 1; $i < $totalPages+1; $i++) {
			$act = ($i == $this->page) ? 'class="active"' : '';
			$html .= '<li sel="'.$i.'" '.$act.'><a href="#"  >'. $i . '</a></li>';
		}



		if ($currentPage < $totalPages) {
			$html.= '<li class="nextpage"  data-page="'.($currentPage+1).'" ><a href=""> >> </a></li>';
		}

		$id = (empty($this->classname)) ? 'id="noSearch"' :  '';

		return '<div class="pagination pagination-right" '.$id.'><ul data-term="'.$this->searchTerm.'" data-class="'.$this->classname.'">' . $html . '</ul></div>';
	}
}

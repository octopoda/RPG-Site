...
/* ================================================
   ...
   ================================================	 */
		...
		public function setPosition ($newPosition, $varName) {
			...
			$posLow = $position; //3
			$posHigh = $newPosition; //4

			if ($posLow > $posHigh) {
				$posLow = $newPosition; //4
				$posHigh = $position; //3
			}


			$db->query("UPDATE {$this->table} SET position = 400000 WHERE position = {$position}");
			$db->query("SELECT @sign:= SIGN({$posHigh}-{$posLow}) FROM {$this->table}");
			//$db->query("SELECT @sign:= SIGN({$position}-{$newPosition}) FROM {$this->table}");
			$db->query("UPDATE {$this->table} SET position = @sign + position WHERE position > {$posLow} AND NOT position = 200 AND NOT position = 400000");
			// $db->query("UPDATE {$this->table} SET position = @sign + position WHERE position BETWEEN {$posLow} AND {$posHigh}");
			$db->query("UPDATE {$this->table} SET position = {$newPosition} WHERE position = 40000");
		...
		public function movePosition($newPosition, $oldPosition, $parent, $menu_id) {
			...
			$db->query("UPDATE {$this->table} SET position = 400000 WHERE position = {$newPosition}");
			$db->query("UPDATE {$this->table} SET position = {$newPosition} WHERE position = {$oldPosition}");
			$db->query("UPDATE {$this->table} SET position = {$oldPosition} WHERE position = 400000");
		...
		public function listAccessGroups() {
			...
			$result_set = $db->queryFill("SELECT * FROM userGroups ORDER BY position");
		...
		public function accessDropDown($id, $navid="") {
			...
			foreach($array as $list){
				if ($list['groupname'] == 'Batman') {
					continue;
				}

				$html .= '<option value="'. $list['position'].'" ';
				if ($id == $list['position']) {

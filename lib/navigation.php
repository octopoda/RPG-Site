...
/*  ===========================================
	...
	static function navigationFromDirectLink($link) {
		global $db;

		$result = $db->queryFill("SELECT navigation_id FROM navigation WHERE directLink = '{$link}'");

		if ($result != false) {
			foreach ($result as $row) {
				return $row['navigation_id'];
			}
		}
	}
	...
	public function buildNavigation($title, $parentName ="") {
	...
	private function externalLink($title) {
		$URL = $this->link;

		$html = '<li';

		if ($title == $this->directLink) {
			$html .= ' class="active" ';
		}
		$html .= '><a href="'.$URL.'" '.$this->hasTarget().'>'.$this->title.'</a>';
	...
	private function contentLink($title, $parentName="") {
		...
		if ($this->subNavList == false) {
			...
			$html .='><a href="'.$URL.'" class="dropdown-toggle" href="#" data-toggle="dropdown">'.$this->title.'<b class="caret"></b></a>';
		}
	...
	public static function navigationFromTitle($title) {
		global $db;
		$nav = NULL;

		$result = $db->queryFill('SELECT * FROM navigation WHERE title = "'.$title.'" LIMIT 1');

		foreach ($result as $r) {
		 	$nav = new Navigation($r['navigation_id']);
		}
		// print_r($nav);
		return $nav;
	}
	...
	========================================= */
		...
		public function createFromForm($post) {
			...
			if ($this->navigation_id != false) {
				...
				// echo 'Old Position'. $oldPosition. '<br /><br />';
				// echo 'New Position'. $this->position. '<br /><br/>';
			...
			//New Navigation Position Set
				...
				$position_set = $db->queryFill("SELECT position FROM navigation WHERE menu_id = {$this->menu_id} AND parent_id = {$this->parent_id} AND position = {$this->position} LIMIT 1"); //Find if there is a position already there/
		...
		public function deleteFromForm() {
			...
			$query = $db->query("UPDATE {$this->table} SET position = position-1 WHERE position > {$this->position}");
...
/*  ===========================================
	...
	========================================= */
		...
		//List all children for Nav Item
		public function listChildren() {
			global $db;
			$navList = array();

			$result = $db->queryFill("SELECT navigation_id FROM navigation WHERE parent_id = {$this->navigation_id} ORDER BY position ASC");

			if ($result != false) {
				foreach ($result as $row) {
					$navList[] = $row['navigation_id'];
				}
			}
		...
		public function positionDropDown($menu_id="", $parent_id="") {
			...
			$html = '<label for="position">Postion (Place After Selection)</label>';
...
/*  ===========================================
	...
	public function movePosition($newPosition, $oldPosition, $parent, $menu_id) {
			global $db;

			$db->query("UPDATE {$this->table} SET position = 400000 WHERE position = {$newPosition} AND parent_id = {$parent} AND menu_id = {$menu_id}");
			$db->query("UPDATE {$this->table} SET position = {$newPosition} WHERE position = {$oldPosition} AND parent_id = {$parent} AND menu_id = {$menu_id}");
			$db->query("UPDATE {$this->table} SET position = {$oldPosition} WHERE position = 400000 AND parent_id = {$parent} AND menu_id = {$menu_id}");

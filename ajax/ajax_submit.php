...
if (isset($_POST['login'])) {
	...
	$err = $user->authenticate($email, $password);
...
if (isset($_POST['firstTime'])) {
	$u_id = Users::FirstTime($_POST['member_number']);

	if ($u_id != false) {
		$u = new Users($u_id);
		$ft = $u->firstTimeMessage();
		$u->emailUser($ft);
		echo 'An email has been sent to the email account you setup with the Academy.';
	} else {
		echo 'The member number entered is not in our system';
	}

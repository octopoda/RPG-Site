<?php
	...
	if(isset($_POST['save'])) {
		...
		if ($_POST['table'] == 'content') {
			...
			$grid->where = "NOT users.user_id = 1";
	...
	function userGroups(&$dataArray) {
		global $grid;

		foreach ($dataArray['rows'] as &$row) {
			$userGroups = new UserGroups($row['group_id']);
			$row['position'] = $userGroups->moveArrows($row['group_id'], $userGroups->position, 'forms/admin/list_roles.php');
		}
	}
